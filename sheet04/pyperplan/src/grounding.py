...
def ground(problem):
    ...
    facts, operators = _reachability_analysis(operators, init)
...
def _reachability_analysis(operators, init):
    reachable = set(init)
    num_reached = 0
    # TODO: should use generalized Dijkstra
    while len(reachable) > num_reached:
        num_reached = len(reachable)
        for op in operators:
            if op.preconditions <= reachable:
                reachable |= op.add_effects

    pruned_operators = []
    for op in operators:
        if op.preconditions <= reachable:
            op.del_effects &= reachable
            assert op.add_effects <= reachable
            pruned_operators.append(op)

    return reachable, pruned_operators
